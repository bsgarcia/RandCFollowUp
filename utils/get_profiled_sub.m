function [heuristic, learning] = get_profiled_sub(dd, de)
    ...
    for k=1:numel(fn)
        if (ismatrix(dd.(fn{k}))) && (numel(dd.(fn{k})) > 2) && ~ischar(dd.(fn{k}))
            try
            heuristic.(fn{k}) = dd.(fn{k})(to_keep, :);
            catch
                disp()
            end
        else
            heuristic.(fn{k}) = dd.(fn{k});
        end
    end
    heuristic.nsub = length(to_keep);
    ...
    for k=1:numel(fn)
        if (ismatrix(dd.(fn{k}))) && (numel(dd.(fn{k})) > 2) && ~ischar(dd.(fn{k}))
            try
            learning.(fn{k}) = dd.(fn{k})(to_keep, :);
            catch
                disp()
            end
        else
            learning.(fn{k}) = dd.(fn{k});
        end
    end
    learning.nsub = length(to_keep);
end



function score = heuristic_choice(data)
