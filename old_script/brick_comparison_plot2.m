...
for n = 1:nbar
    ...
    fill([x_values(n)+width x_values(n)+Wbar x_values(n)+Wbar x_values(n)+width],...
        ...
        set_alpha(color1, .23),...
        'EdgeColor', 'none', 'linewidth', .1);%,...%trace,...
        %'FaceAlpha',0.23);
    hold on
    %disp(width)
    fill([x_values(n)+width x_values(n)+Wbar x_values(n)+Wbar x_values(n)+width],...
        [curve-sem curve-sem curve+sem curve+sem],...
        set_alpha(color1, .7), 'linewidth', .1);%,...
...
%     hold on
    ...
    yMean = [curve; curve];
    plot(xMean,yMean,'-','LineWidth',.6,'Color',color1);
    hold on
...
%     hold on
    ...
    fill([x_values(n)-width x_values(n)-Wbar x_values(n)-Wbar x_values(n)-width],...
        [curve-sem*conf curve-sem*conf curve+sem*conf curve+sem*conf],...
        set_alpha(color2, .23),...
        'edgecolor', 'black', 'linewidth', .1);%,...%trace,...
        %'FaceAlpha',0.23);
    hold on
    ...
    fill([x_values(n)-width x_values(n)-Wbar x_values(n)-Wbar x_values(n)-width],...
        [curve-sem curve-sem curve+sem curve+sem],...
        set_alpha(color2, .7), 'linewidth', .1);%,...
...
%     hold on
    ...
    xMean = [x_values(n)-width ; x_values(n)-Wbar];
    yMean = [curve; curve];
    plot(xMean,yMean,'-','LineWidth',0.6,'Color',color2);
    hold on
