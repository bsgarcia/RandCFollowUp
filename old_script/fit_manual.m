...
for exp_num = selected_exp
    ...
    fit_params.ncond = 4;
    fit_params.alpha_values = linspace(0, .1, 50);
    
    save_params.fit_file = sprintf(...
        '%s%s%s%d', fit_folder, exp_name,  '_learning_manual_', sess);
...
function [parameters,ll] = ...
    ...
    parameters = cell(length(fit_params.models));
    for i = 1:length(fit_params.models)
        parameters{i} = nan(fit_params.nsub, 2);
    end
  
    ll = nan(length(fit_params.models), fit_params.nsub);
    ...
    for sub = 1:fit_params.nsub
        ...
        for model = fit_params.models
            ...
            l1 = nan(1, length(fit_params.alpha_values));
            i = 0;
            for value = fit_params.alpha_values
               i = i + 1;
            ...
            where = find(l1 == min(l1));
            idx_min = where(randi(length(where)));
            
            parameters{model}(sub, :) =  [1, fit_params.alpha_values(idx_min)];
            ll(model, sub) = l1(idx_min);
